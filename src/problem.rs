...
impl Display for Problem {
	fn fmt(&self, f: &mut Formatter<'_>) -> Result {
		match self {
			AdventOfCode(year, day, part) => write!(f, "Advent of Code {} Day {} Part {}", year, day, part),
			None => write!(f, "None"),
		}
	}
}
...
impl From<&str> for Problem {
	fn from(s: &str) -> Self {
		...
		if problem.len() != 2 {
			eprintln!("Invalid problem format: {}. Use ':' as separator: problem_type:problem_id.", s);
		}
		match problem[0] {
...
fn parse_advent_of_code (s: &str) -> Problem {
	...
	if problem.len() != 3 {
		eprintln!("Invalid problem format: {}. Use '-' as separator: year-day-part.", s);
		return None;
	}
	...
	if year.is_err() || day.is_err() || part.is_err() {
		eprintln!("Can't parse problem data: {}. Problem id must contain only numbers.", s);
		return None;
	}
